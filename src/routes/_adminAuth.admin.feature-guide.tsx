...
const CUSTOMER_FACING: GuideGroup = {
  ...
  entries: [
    ...
    },
    {
      ...
      description: "Every account gets a shareable referral link/code. When a referred friend signs up and later places a paid order, both sides are credited with points — sized by Referral Payout Tiers (Admin & Operations). If no payout tier is configured, referrals sign up fine but never actually pay out.",
    },
    {
...
const ADMIN_OPS: GuideGroup = {
  ...
  entries: [
    ...
    { name: "Referral Payout Tiers (admin)", description: "Defines how many points the referrer and the referred customer each get when a referral's first order is confirmed, sized by that order's value. Required for the give/get referral program to pay out at all — must have at least one active tier.", route: "/admin/referral-tiers" },
    ...
    { name: "Credit Accounts", description: "A \"Coming soon\" placeholder for a future trade-credit product (buy-on-account with a credit limit and payment terms) — not the rewards points system, and not live yet.", route: "/admin/credit-accounts" },
